...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($token)) {
        ...
        die("CSRF Token Validation Failed. <br>Form Token: $token <br>Session Token: $sessionToken");
    }
}
...
if ($subRoute === 'password-update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$user) {
        set_flash_message("Session expired. Please login again.", "error");
    ...
    if (!$uData) {
        set_flash_message("User account not found.", "error");
        header("Location: " . url('admin/profile?error=user_not_found'));
        exit;
    }

    if (!password_verify($currentPass, $uData['password_hash'])) {
        set_flash_message("Incorrect current password.", "error");
        header("Location: " . url('admin/profile?error=invalid_current'));
        exit;
    }

    if ($newPass !== $confirmPass) {
        set_flash_message("New passwords do not match.", "error");
        header("Location: " . url('admin/profile?error=match_mismatch'));
        exit;
    }

    if (strlen($newPass) < 6) {
        set_flash_message("Password must be at least 6 characters.", "error");
        header("Location: " . url('admin/profile?error=too_short'));
        exit;
    }
    ...
    if ($stmt->rowCount() > 0) {
        ...
        header("Location: " . url('admin/profile?updated=1'));
    } else {
        set_flash_message("No changes were made to the password.", "info");
